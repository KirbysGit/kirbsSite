...
const SkillTower = memo(function SkillTower({
  ...
  overlap = 0,   	// example: -20 to overlap the next tower a bit
  ...
  leftPos = 0    	// horizontal position as percentage of parent width (0-100)
	...
	// deterministic lit windows. (how we randomly choose which windows are lit)
	const lit = useMemo(() => {
		const set = new Set();
	...
	// atmospheric perspective for distant buildings (how we make the buildings look further away)
	...
	// this is the height of the top of the building.
	const capHeight = 
		...
		>
			...
			>
				...
				{cap === "spire" && (() => {
					...
					const tiers = 5;											// # of tiers.
					const stepH = Math.max(7, Math.round(H * 0.025));			// height of each tier.
					const inset0 = Math.max(4, Math.round(faceW * 0.08));		// initial inset.
					const insetStep = Math.max(2, Math.round(faceW * 0.04));	// step size.
					const mastH = Math.max(50, Math.round(H * 0.12));			// height of the mast.
					const mastW = Math.max(4, Math.round(faceW * 0.08));		// width of the mast.
					...
					return (
						...
						{Array.from({ length: tiers }).map((_, i) => {
							const inset = inset0 + i * insetStep;								// inset for each tier.
							const yTop = -5 - (i + 1) * stepH;									// y position of each tier.
							const wFront = Math.max(8, faceW - inset * 2);						// width of each tier.
							const xFront = faceX + inset;										// x position of each tier.
				...
				{cap === "billboard" && (() => {
					const w = Math.max(56, faceW * 0.8); 	// width of the billboard.
					const h = 22; 							// height of the billboard.
					const x = faceX + (faceW - w) / 2; 		// x position of the billboard.
					const y = -h - 11; 						// y position of the billboard.
					return (
				...
				{cap === "gable" && (() => {
					...
					const upperW = faceW - inset * 2; 						// width of upper section.
					const upperX = faceX + inset; 							// x position of upper section.
					const roofPeakY = -parapetH - upperH - roofH; 			// y position of roof peak.
					const roofBaseY = -parapetH - upperH; 					// y position of roof base.
					...
					return (
						...
						<g>
							...
							{(() => {
								const winW = 4; 																// width of each window.
								const winH = 6; 																// height of each window.
								const winGap = 7; 																// gap between windows.
								const winCount = Math.floor((upperW - 8) / (winW + winGap)); 					// number of windows.
								const startX = upperX + (upperW - (winCount * (winW + winGap) - winGap)) / 2; 	// x position of the first window.
								const winY = roofBaseY + (upperH - winH) / 2; 									// y position of the first window.
				...
				{cap === "ac" && (() => {
					...
					const curbH = 4; // height of the curb.
					const unitH = 8; // height of the AC unit.
				...
				{cap === "dome" && (() => {
					...
					const cx     = faceX + faceW/2;							// center of the dome.
					const left   = cx - domeW/2;							// left side of the dome.
					const right  = cx + domeW/2;							// right side of the dome.
					const apexY  = baseY - domeH;							// apex of the dome.
					const clipId = `${id}-domeClip`;						// clip path for the dome.
					...
					return (
						<g aria-label="dome">
							...
							<g clipPath={`url(#${clipId})`}>
								...
								const w = Math.max(1.2, 2 - normalizedDist * 0.7);	// width of the rib.
								const op = Math.max(.55, 1 - normalizedDist * 0.4);	// opacity of the rib.
